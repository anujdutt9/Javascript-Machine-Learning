...
class Data {
  constructor(bs, mseed, base_url='/data/json_features/', data_len=1441, min=720) {
    this.batch_size=bs;
    this.mseed=mseed;
  ...
  async data2tensor(data, batch_size, max_columns=COLUMN_SIZE, num_classes=NUM_CLASSES) {
    ...
    var xs = tf.tensor1d(x, 'float32').reshape([batch_size, max_columns, 1]);
    var xs = this.mseed.predict(xs);
...
async function get_model(learning_rate, decay=1e-6, NUM_CLASSES=10) {
  const pretrained = await tf.loadModel('/models/speech_emotions_detection_model.json');
  console.log(pretrained.layers);
  // Return a model that outputs an internal activation.
  const layer = pretrained.getLayer('conv1d_Conv1D6');

  const model_seed=tf.model({inputs: pretrained.inputs, outputs: layer.output});
  ...
  const model = tf.sequential({
    layers: [
      ...
      // parameters).
      tf.layers.flatten({
        inputShape: model_seed.outputs[0].shape.slice(1)
      }),
